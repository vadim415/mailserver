...
openssl req \
  ...
  -keyout "${DIR}/dovecot.ca.key" \
  -out "${DIR}/dovecot.ca.crt"
#
...
# the bits to 4096.
openssl genrsa -out "${DIR}/dovecot.key" 2048

# Generate a CSR using the configuration and the key just generated. We will
# give this CSR to our CA to sign.
openssl req \
  -new -key "${DIR}/dovecot.key" \
  -out "${DIR}/dovecot.csr" \
...
openssl x509 \
  ...
  -days 3650 \
  -in "${DIR}/dovecot.csr" \
  -CA "${DIR}/dovecot.ca.crt" \
  -CAkey "${DIR}/dovecot.ca.key" \
  -CAcreateserial \
  -extensions v3_req \
...
openssl x509 -in "${DIR}/dovecot.crt" -noout -text
