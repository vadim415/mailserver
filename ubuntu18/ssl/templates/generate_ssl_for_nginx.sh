...
openssl req \
  ...
  -keyout "${DIR}/nginx.ca.key" \
  -out "${DIR}/nginx.ca.crt"
#
...
# the bits to 4096.
openssl genrsa -out "${DIR}/nginx.key" 2048

# Generate a CSR using the configuration and the key just generated. We will
# give this CSR to our CA to sign.
openssl req \
  -new -key "${DIR}/nginx.key" \
  -out "${DIR}/nginx.csr" \
...
openssl x509 -in "${DIR}/nginx.crt" -noout -text
