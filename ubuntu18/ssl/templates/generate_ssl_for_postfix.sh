...
openssl req \
  ...
  -keyout "${DIR}/postfix.ca.key" \
  -out "${DIR}/postfix.ca.crt"
#
...
# the bits to 4096.
openssl genrsa -out "${DIR}/postfix.key" 2048

# Generate a CSR using the configuration and the key just generated. We will
# give this CSR to our CA to sign.
openssl req \
  -new -key "${DIR}/postfix.key" \
  -out "${DIR}/postfix.csr" \
...
openssl x509 \
  ...
  -days 3650 \
  -in "${DIR}/postfix.csr" \
  -CA "${DIR}/postfix.ca.crt" \
  -CAkey "${DIR}/postfix.ca.key" \
  -CAcreateserial \
  -extensions v3_req \
...
openssl x509 -in "${DIR}/postfix.crt" -noout -text
