...
openssl req \
  ...
  -nodes \
  -x509 \
  -subj "/C=DE/ST=Munhen/L=Ubn24/O=My Company CA" \
  -keyout "${DIR}/postgresql.ca.key" \
  -out "${DIR}/postgresql.ca.crt"
#
...
# the bits to 4096.
openssl genrsa -out "${DIR}/postgresql.key" 2048

# Generate a CSR using the configuration and the key just generated. We will
# give this CSR to our CA to sign.
openssl req \
  -new -key "${DIR}/postgresql.key" \
  -out "${DIR}/postgresql.csr" \
...
openssl x509 \
  -req \
  -days 3650 \
  -in "${DIR}/postgresql.csr" \
  -CA "${DIR}/postgresql.ca.crt" \
  -CAkey "${DIR}/postgresql.ca.key" \
  -CAcreateserial \
  -extensions v3_req \
  -extfile "${DIR}/openssl.cnf" \
  -out "${DIR}/postgresql.crt"

# (Optional) Verify the certificate.
openssl x509 -in "${DIR}/postgresql.crt" -noout -text
